...
if ($role === 'admin' || !empty($courseIds)) {
    ...
    if ($role !== 'admin') {
        ...
        $queryTugas .= " WHERE m.course_id IN ($in)";
    }

    $queryTugas .= " ORDER BY a.tanggal_batas_akhir ASC";

    $resultTugas = mysqli_query($conn, $queryTugas);
    $totalTugas = mysqli_num_rows($resultTugas);
}

// ==============================
// Ambil Jadwal Kursus (Untuk Forum)
// ==============================
$resultJadwal = null;
$totalJadwal = 0;
...
if ($role === 'admin') {
    $queryJadwal = "
        SELECT s.schedule_id, s.course_id, c.nama_kursus, s.tanggal_mulai, s.tanggal_selesai, s.hari_pelaksanaan
        FROM schedules s
...
$totalJadwal = mysqli_num_rows($resultJadwal);
?>
...
<body>
    ...
    <h1 class="page-title">
        <i class="fas fa-calendar-alt me-3"></i>Daftar Kursus & Jadwal
    </h1>

   <div class="main-container">
    <h1 class="page-title">
        <i class="fas fa-calendar-alt me-3"></i>Daftar Jadwal Kursus
    </h1>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card task-card h-100">
                        <div class="card-body d-flex flex-column">
                            ...
                            <h5 class="card-title"><?= htmlspecialchars($row['nama_kursus']) ?></h5>

                            <div class="info-item">
                                <div class="info-icon subject-icon"><i class="fas fa-calendar"></i></div>
                            ...
                            <div class="info-item">
                                ...
                                <div class="info-content">
                                    ...
                                    <div class="info-value"><?= htmlspecialchars($row['hari_pelaksanaan']) ?></div>
                                </div>
                            </div>

                            <div class="mt-auto">
                                <a href="forum_diskusi.php?course_id=<?= $row['course_id'] ?>" class="action-btn btn btn-primary w-100">
                                    <i class="fas fa-comments me-2"></i>Masuk Forum Diskusi
                                </a>
                            </div>
    ...
    <?php else: ?>
        <div class="empty-state" id="emptyState">
            ...
            <p>Belum ada jadwal mengajar atau kursus untuk anda saat ini.</p>
        </div>
    <?php endif; ?>
</div>
